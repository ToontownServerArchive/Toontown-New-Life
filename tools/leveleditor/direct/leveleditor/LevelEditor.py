...
# or you can hack this up for your own purposes.
    ...
    hoodString = base.config.GetString('level-editor-hoods', 'TT DD BR DG DL MM CC CL CM CS GS GZ OZ PA TUT ES CW FF CZ PP')
...
hoodIds = {'TT': 'toontown_central',
           ...
           'TUT': 'tutorial',
           'ES': 'estate',
           'CW': 'dessert_playground',
		   'FF': 'funny_farm',
		   'CZ': 'construction_zone',
		   'PP': 'beach_playground'
           }
...
try:
    ...
    if 'OZ' in hoods:
        ...
        loadDNAFile(DNASTORE, 'phase_14/dna/storage_OZ_town.dna', CSDefault, 1)
    if 'GZ' in hoods:
    ...
    if 'CW' in hoods:
        loadDNAFile(DNASTORE, 'phase_14/dna/storage_CW.dna', CSDefault, 1)
    if 'FF' in hoods:
        loadDNAFile(DNASTORE, 'phase_14/dna/storage_FF.dna', CSDefault, 1)
        loadDNAFile(DNASTORE, 'phase_14/dna/storage_FF-sz.dna', CSDefault, 1)
        loadDNAFile(DNASTORE, 'phase_14/dna/storage_FF_town.dna', CSDefault, 1)
    if 'CZ' in hoods:
        loadDNAFile(DNASTORE, 'phase_14/dna/storage_CZ.dna', CSDefault, 1)
        loadDNAFile(DNASTORE, 'phase_14/dna/storage_CZ_sz.dna', CSDefault, 1)
        loadDNAFile(DNASTORE, 'phase_14/dna/storage_CZ_town.dna', CSDefault, 1)
    if 'PP' in hoods:
        loadDNAFile(DNASTORE, 'phase_14/dna/storage_PP.dna', CSDefault, 1)
